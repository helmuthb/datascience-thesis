...
class Augment():
    ...
    def __call__(self, image, boxes_cl, boxes_yx, mask, name):
        def _tuple_float(t):
        ...
        t = self.t
        boxes = [list(b) + [str(c)] for c, b in zip(boxes_cl, boxes_yx)]
        out = t(
            image=image,
            bboxes=boxes,
            mask=mask
        ...
        bout_yx = np.array([b[:4] for b in out['bboxes']], np.single)
        return (out['image'].astype(np.single),
                bout_cl, bout_yx,
                out['mask'], name)

    def tf_wrap(self):
        def _tf_wrap(image, boxes_cl, boxes_yx, mask, has_mask, name):
            image, cl, yx, mask, name = tf.numpy_function(
                self,
                (image, boxes_cl, boxes_yx, mask, name),
            ...
            return (image, cl, yx, mask, has_mask, name)
        return _tf_wrap
